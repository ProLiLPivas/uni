...
public class AStarState
{
     ...
     **/
    public boolean addOpenWaypoint(Waypoint newWауPoint) {
        for (Map.Entry<Location, Waypoint> entry : HashMapOpen.entrySet()) {
            if (entry.getKey().equals(newWауPoint.getLocation())) {
                if (entry.getValue().getRemainingCost() > newWауPoint.getRemainingCost()){
                    HashMapOpen.put(newWауPoint.getLocation(), newWауPoint);
                    return true;
                } else {
                    return false;
                }
